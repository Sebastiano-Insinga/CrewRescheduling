...
def write_twan_format(output_file, duties, tasks):
    ...
    for _, task_ids in duties.items():
...
def main():
    ...
    for csv_file in csv_files:
        ...
        # Estrai il nome base (es: Transformed-S01)
        instance_name = csv_name.replace("_duties_with_tasks.csv", "")

        # Trova il file TSV corrispondente via S_ID_MAPPING
        s_id = None
        ...
        instance_file = None
        if s_id:
            candidate = INSTANCE_DIR / f"Transformed-{s_id}.tsv"
            if candidate.exists():
                instance_file = candidate

        if instance_file is None or not instance_file.exists():
            print(f"⚠ SKIP: {csv_name}")
            print(f"   → Istanza non trovata per: {instance_name}")
            failed += 1
            continue

        # File di output
        txt_output = TXT_OUTPUT_DIR / f"{instance_name}_sol.txt"
        
        print(f"Elaborazione: {csv_name}")
        
        try:
            # Leggi istanza
            tasks = read_instance(str(instance_file))

            # Leggi CSV
            duties = read_duties_csv(str(csv_file))

            # Converti
            write_twan_format(str(txt_output), duties, tasks)
