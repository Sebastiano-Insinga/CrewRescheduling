...
- [Validator_design.md](Validator_design.md) — validatore ex-post delle soluzioni: perché le regole sono riscritte invece che importate dal solver, split per dominio (`Helper`/`CrewChecks`/`LocoChecks`), catalogo dei vincoli crew (C1-C7) e loco (L1-L7) con stato e funzione, semantica del carryover e delle finestre per la pausa, bug del solver trovati sulle pause e correzioni.
